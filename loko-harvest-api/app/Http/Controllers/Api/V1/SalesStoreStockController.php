    ...
    public function index(Request $request)
    {
        ...
        if ($date && $date !== now()->toDateString()) {
            $stock = $stockQuery->get()->map(function ($item) use ($date) {
                ...
                $transfersSalesInAfter = (float) (clone $transfersSalesInQuery)->where('transfer_date', '>', $date)->sum('quantity');
                $transfersSalesInOn = (float) (clone $transfersSalesInQuery)->where('transfer_date', '=', $date)->sum('quantity');

                $transfersInAfter = $transfersProdAfter + $transfersSalesInAfter;
                ...
                $conversionsInAfter = (float) (clone $conversionsInQuery)->where('created_at', '>', $date . ' 23:59:59')->sum('to_quantity');
                ...
                $conversionsOutAfter = (float) (clone $conversionsOutQuery)->where('created_at', '>', $date . ' 23:59:59')->sum('from_quantity');
                ...
                $transfersSalesOutAfter = (float) (clone $transfersSalesOutQuery)->where('transfer_date', '>', $date)->sum('quantity');
                ...
                $soldAfter = (float) (clone $soldQuery)->where('orders.order_date', '>', $date)->sum('order_items.quantity');
                ...
                $replacementsAfter = (float) (clone $replacementsQuery)->where('created_at', '>', $date . ' 23:59:59')->selectRaw('SUM(allocated_quantity - returned_quantity) as total')->value('total') ?? 0;
                ...
                $closingStockOnD = $liveClosing - ($transfersInAfter + $conversionsInAfter) + ($conversionsOutAfter + $transfersSalesOutAfter + $soldAfter + $replacementsAfter + $damagesAfter);
