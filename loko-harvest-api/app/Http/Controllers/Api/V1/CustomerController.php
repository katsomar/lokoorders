...
class CustomerController extends Controller
{
    ...
    {
        $customer = Customer::with(['zone', 'account', 'parent', 'branches.account', 'branches.zone'])->findOrFail($id);
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'parent_id' => 'required_if:classification,branch|nullable|uuid|exists:customers,id',
            'contact_person' => 'required|string|min:2|max:100',
            'phone_primary' => 'required|string|regex:/^[0-9+\-\s()]{9,20}$/',
            'phone_secondary' => 'nullable|string|regex:/^[0-9+\-\s()]{9,20}$/',
    ...
    public function update(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            ...
            'parent_id' => 'required_if:classification,branch|nullable|uuid|exists:customers,id',
            'contact_person' => 'sometimes|required|string|min:2|max:100',
            'phone_primary' => 'sometimes|required|string|regex:/^[0-9+\-\s()]{9,20}$/',
            'phone_secondary' => 'nullable|string|regex:/^[0-9+\-\s()]{9,20}$/',
            'email' => 'nullable|email|max:255',
