...
| 2026-07-08 | **Damages Tracking** | Added batch_reference to Production adjustments, split inputs, Damages columns in all ledgers, scaled signature coords with programmatic touch listeners, and click-to-view Admin justification dialogs with loss values and image lightboxes. | Keep balances synced per-batch, simplify egg quantity reporting, track loss visibility, resolve touch offsets, prevent console passive listener errors, and provide Admin with comprehensive historical visual audit references. |
...
* **The Implementation & Rationale**:
  ...
  - **Admin Click-to-View Justification Modal**:
    - Refactored `StoreAdjustmentController@index` to support additional filters (`product_id`, `batch_reference`, `adjustment_date`) and a `per_page = -1` bypass to load full collections.
    - Rendered the damage column numbers in a distinct red font color (`text-red-655`) to indicate a loss state.
    - Enabled interactive click-triggers on non-zero damages cells in both `production-store/page.tsx` and `sales-store/page.tsx`. Clicking a cell loads adjustment details, calculating the total financial loss value (quantity * price), identifying who reported and approved the record, displaying the reason notes, and hosting magnifier lightboxes for visual verification (images and signatures).
